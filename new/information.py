...
class Input:
    ...
    def get_input(self, env):
        ...
        # 测试环境信息
        if env == 'test':
            return self.get_test_input()
        # 线上环境信息
        elif env == 'online':
            return self.get_online_input()
        # 输入错误则报错
        else:
    ...
    def get_code_view_config_path(self, env):
        """根据主程序/配置文件中的环境标示来获取不同配置文件路径"""
        # 测试环境
        if env == 'text':
            return self.mock_test_config_path()
        elif env == 'online':
            return self.online_config_path()
        else:
            raise ValueError('unrecognized env str')